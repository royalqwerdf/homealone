...
import com.elice.homealone.module.recipe.repository.IngredientRepository;
...
public class IngredientService {
    ...
    public Ingredient addIngredient(RecipeIngredient recipeIngredient) {
        ...
        Ingredient ingredient = similarIngredientExists(name);
        ingredient.addIngredient(recipeIngredient);
        return ingredient;
    }

    // RecipeIngredient와 이름이 유사한 Ingredient가 있는지 확인하는 메소드
    // name에서 영어와 한글, 숫자만 남기고, 소문자는 대문자로 치환해서 비교
    public Ingredient similarIngredientExists(String name) {
        ...
        Optional<Ingredient> ingredient = ingredientRepository.findByName(newName);
